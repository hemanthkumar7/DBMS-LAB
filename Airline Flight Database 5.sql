...
WHERE A.Aid IN (SELECT C.aid
 ...
 /*	For each pilot who is certified for more than three aircrafts, 
...
/*	For all aircraft with cruising range over 1000 Kms, 
...
WHERE A.cruisingrange >( SELECT MIN (F.distance)
 ...
 /*A customer wants to travel from Bangalore to Delhi  with no more than two changes of flight. 
 List the choice of departure times from Bangalore if the customer wants to arrive in Delhi  by 6 p.m. */
SELECT F.departs
FROM Flights F
